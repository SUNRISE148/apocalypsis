...
wawe1 = font1.render("WAWE 1", True, (180,0,0))
wawe2 = font1.render("WAWE 2", True, (180,0,0))
wawe3 = font1.render("WAWE 3", True, (180,0,0))
rage = font1.render("RAAAAGE", True, (180,0,0))
...
class Player(GameSprite):
    ...
    def fire_left(self):
        bullet = Bullet(img_bullet_left, self.rect.centerx-46, 418,20,15, -15)
        bullets.add(bullet)
    def fire_right(self):
...
class Enemy(GameSprite):
    def update(self):
        ...
        if self.rect.x < 350 and self.rect.x > 300:
            a = randint(0,1)
            if self.imaage == 'enemy.png':
                self.rect.x = randint(750,850)
                self.speed = randint(1,5)*-1
            if self.imaage == 'enemy_right.png':
                self.rect.x = randint(-150,-50)
...
ship = Player(img_hero_left,300,370,100,125,12)
life_station = GameSprite('life.png', 300, 250,100,100,0)
monsters = sprite.Group()
clip = sprite.Group()
window.blit(wawe1,(260,460))
for i in range(1,kol_vo_en):
    a = randint(0,1)
    if a == 0:
        monster = Enemy(img_enemy,randint(770,850), 360,50,115,randint(min_speed,max_speed)*-1)
        monsters.add(monster)
    if a == 1:
        monster = Enemy(img_enemy_right,randint(-150,-50), 360,50,115,randint(min_speed,max_speed))
        monsters.add(monster)
for i in range(1,patrons+1):
...
while run:
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_SPACE:
                ...
                for b in clip:
                            i+=1
                            if i == ((patrons) -num_fire):
                                b.kill()
                if fire_direction == 1:
                    if num_fire < patrons and rel_time == False:
                        num_fire = num_fire +1
                        fire_sound.play()
                        i = 0
                        ship.fire_left()
    ...
    if not finish:
        ...
        for c in collides:
            ...
            a = randint(0,1)
            if a == 0:
                monster = Enemy(img_enemy, randint(750,850), 360,50,115,randint(min_speed,max_speed)*-1)
                monsters.add(monster)
            if a == 1:
                monster = Enemy(img_enemy_right,randint(-150,-50), 360,50,115,randint(min_speed,max_speed))
                monsters.add(monster)
        ...
        if score >= goal:
            finish = True
            if wawe+1 == 2:
                window.blit(wawe2,(260,200))
            if wawe+1 == 3:
                window.blit(wawe3,(260,200))
            if wawe+1 == 4:
                window.blit(rage,(260,200))
            if wawe+1 == 5:
